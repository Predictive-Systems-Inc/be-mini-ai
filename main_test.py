    ...
    try:
        ...
        if "setup" in message:
            ...
            await manager.send_message(client_id, {
                "serverContent": {
                    "modelTurn": {
                        "parts": [{"text": "Audio testing server ready. Start speaking to test audio file saving."}]
                    },
                    "turnComplete": True
                }
            })
                            ...
                            if (state["accumulated_audio"] and 
                                ...
                                try:
                                    ...
                                    if filepath:
                                        ...
                                        voice_response = await send_voice_file_to_client(client_id)
...
async def send_voice_file_to_client(client_id: str) -> dict:
    """
    Read the voice file and send it as PCM data to the client.
    """
    try:
        voice_filepath = "./voice/a_projectile_is.wav"
